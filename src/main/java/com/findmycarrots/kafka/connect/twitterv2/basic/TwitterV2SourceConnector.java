...
public class TwitterV2SourceConnector extends SourceConnector {
  private static Logger log = LoggerFactory.getLogger(TwitterV2SourceConnector.class);
  ...
  public void start(Map<String, String> map) {
    this.config = new TwitterV2SourceConnectorConfig(map);
    this.settings = map;
  }
  ...
  public List<Map<String, String>> taskConfigs(int maxTasks) {
    ...
    for (Integer taskID : taskToKeywords.keySet()) {
      ...
      taskSettings.put(TwitterV2SourceConnectorConfig.FILTER_KEYWORDS_CONF, Joiner.on(',').join(keywords));
  ...
  public ConfigDef config() {
    return TwitterV2SourceConnectorConfig.conf();
  }
}
